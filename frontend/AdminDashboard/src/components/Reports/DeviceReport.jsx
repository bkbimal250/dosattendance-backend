import React from 'react';
import { Card } from '../ui';
import { Activity, Wifi, Database } from 'lucide-react';

const DeviceReport = ({ data }) => {
  if (!data) return null;

  return (
    <Card className="p-6">
      <h2 className="text-xl font-semibold text-gray-900 mb-4">Device Report</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="text-center">
          <Activity className="w-8 h-8 text-blue-600 mx-auto mb-2" />
          <div className="text-2xl font-bold text-gray-900">
            {data.devices?.length || 0}
          </div>
          <div className="text-sm text-gray-600">Total Devices</div>
        </div>
        <div className="text-center">
          <Wifi className="w-8 h-8 text-green-600 mx-auto mb-2" />
          <div className="text-2xl font-bold text-gray-900">
            {data.devices?.filter(device => device.connection_status === 'connected').length || 0}
          </div>
          <div className="text-sm text-gray-600">Connected Devices</div>
        </div>
        <div className="text-center">
          <Database className="w-8 h-8 text-purple-600 mx-auto mb-2" />
          <div className="text-2xl font-bold text-gray-900">
            {data.devices?.reduce((acc, device) => acc + (device.recent_attendance_24h || 0), 0) || 0}
          </div>
          <div className="text-sm text-gray-600">24h Activity</div>
        </div>
      </div>
    </Card>
  );
};

export default DeviceReport;
